...
The catch-up worker probes each stream with a lightweight `SELECT ... LIMIT 1` query. On streams with many expired tombstones, these probes can be slow. You can configure the probe timeout (default 30 seconds). Note that the timeout applies per candidate task (all stream probes for a task are dispatched concurrently), not per individual stream probe:
