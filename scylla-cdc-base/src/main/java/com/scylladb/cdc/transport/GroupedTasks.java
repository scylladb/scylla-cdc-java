...
public class GroupedTasks {
    ...
    public GroupedTasks(Map<TaskId, SortedSet<StreamId>> tasks, GenerationMetadata generationMetadata) {
        ...
        this.startReadTimestamp = Optional.empty();
        this.endReadTimestamp = Optional.empty();
    }
    ...
    public Timestamp getStartReadTimestamp() {
        if (startReadTimestamp.isPresent()) {
            return startReadTimestamp.get();
        }
        // If not explicitly set, return the generation start timestamp
        return generationMetadata.getStart();
    }
