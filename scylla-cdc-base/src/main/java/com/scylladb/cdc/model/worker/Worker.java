...
public final class Worker {
    ...
    private static final class TaskStateMigration {
        ...
        private final Map<CoordinationGroup<TaskId, TaskId>, Set<TaskId>> coordinationProgress;

        private TaskStateMigration(
                Set<TaskId> legacyTasks,
                Set<TaskId> restoredTasks,
                Set<CoordinationGroup<TaskId, TaskId>> coordinationGroups,
                Map<CoordinationGroup<TaskId, TaskId>, Set<TaskId>> coordinationProgress) {
            ...
            this.coordinationGroups = Collections.unmodifiableSet(
                    new HashSet<>(coordinationGroups));
            Map<CoordinationGroup<TaskId, TaskId>, Set<TaskId>> progressCopy = new HashMap<>();
            coordinationProgress.forEach((group, participants) -> progressCopy.put(
                    group, Collections.unmodifiableSet(new HashSet<>(participants))));
            this.coordinationProgress = Collections.unmodifiableMap(progressCopy);
        }
    }
    ...
    private static final class TabletMigrationPlan {
        ...
        private final Map<TaskId, CoordinationGroup<TaskId, TaskId>> coordinationByLegacyTask;

        private TabletMigrationPlan(Set<TaskId> assignedTasks,
                                    Map<TaskId, Set<TaskId>> replacementsByLegacyTask,
                                    Map<TaskId, CoordinationGroup<TaskId, TaskId>>
                                            coordinationByLegacyTask) {
            this.assignedTasks = assignedTasks;
            this.replacementsByLegacyTask = replacementsByLegacyTask;
            this.coordinationByLegacyTask = coordinationByLegacyTask;
        }

        private static TabletMigrationPlan create(GroupedTasks workerTasks) {
            ...
            Map<TaskId, CoordinationGroup<TaskId, TaskId>> coordinationByLegacyTask =
                    new HashMap<>();
            workerTasks.getCoordinationGroups().stream()
                    .filter(group -> CoordinationNamespaces.TABLET_TASK_STATE_MIGRATION.equals(
                            group.getNamespace()))
                    .forEach(group -> {
                        Set<TaskId> assignedReplacements =
                                replacementsByLegacyTask.get(group.getKey());
                        if (assignedReplacements == null) {
                            return;
                        }
                        Preconditions.checkArgument(
                                group.getParticipants().containsAll(assignedReplacements),
                                "Tablet migration coordination group %s does not contain every "
                                        + "replacement assigned to this worker",
                                group.getKey());
                        Preconditions.checkArgument(group.getParticipants().stream().allMatch(
                                        taskId -> taskId.isTabletStreamTask()
                                                && taskId.getGenerationId().equals(
                                                        group.getKey().getGenerationId())
                                                && taskId.getTable().equals(
                                                        group.getKey().getTable())),
                                "Tablet migration coordination group %s contains an invalid "
                                        + "replacement task",
                                group.getKey());
                        CoordinationGroup<TaskId, TaskId> previous =
                                coordinationByLegacyTask.put(group.getKey(), group);
                        Preconditions.checkArgument(previous == null,
                                "Duplicate tablet migration coordination group: %s",
                                group.getKey());
                    });
        ...
        private Map<CoordinationGroup<TaskId, TaskId>, Set<TaskId>> coordinationProgress(
                Set<TaskId> returnedLegacyTasks) {
            Map<CoordinationGroup<TaskId, TaskId>, Set<TaskId>> result = new HashMap<>();
            returnedLegacyTasks.forEach(legacyTask -> {
                CoordinationGroup<TaskId, TaskId> group =
                        coordinationByLegacyTask.get(legacyTask);
                if (group != null) {
                    result.put(group, replacementsByLegacyTask.get(legacyTask));
                }
            });
            return result;
        }

        private Set<CoordinationGroup<TaskId, TaskId>> coordinationGroups() {
            return Collections.unmodifiableSet(
                    new HashSet<>(coordinationByLegacyTask.values()));
        }
    ...
    private PreparedTasks prepareTasksWithState(GroupedTasks workerTasks)
        ...
        Map<TaskId, SortedSet<StreamId>> taskMap = workerTasks.getTasks();
        LoadedTaskStates loaded = loadTaskStatesForPreparation(
                workerConfiguration.transport, workerTasks);
    ...
    private static LoadedTaskStates loadTaskStatesForPreparation(
            WorkerTransport transport, GroupedTasks workerTasks) {
        Map<TaskId, SortedSet<StreamId>> taskMap = workerTasks.getTasks();
        TabletMigrationPlan migrationPlan = TabletMigrationPlan.create(workerTasks);
        ...
        Map<CoordinationGroup<TaskId, TaskId>, Set<TaskId>> coordinationProgress =
                migrationPlan.coordinationProgress(returnedLegacyTaskIds);
        Set<TaskId> uncoordinatedLegacyTasks = new HashSet<>(returnedLegacyTaskIds);
        coordinationProgress.keySet().stream()
                .map(CoordinationGroup::getKey)
                .forEach(uncoordinatedLegacyTasks::remove);
        if (!uncoordinatedLegacyTasks.isEmpty()) {
            logger.atWarning().log(
                    "Keeping legacy tablet checkpoints because the worker assignment has no "
                            + "authoritative coordination manifest: %s",
                    uncoordinatedLegacyTasks);
        }
        return new LoadedTaskStates(loadedStates,
                new TaskStateMigration(
                        returnedLegacyTaskIds, restoredTasks,
                        migrationPlan.coordinationGroups(), coordinationProgress));
    }
    ...
    private static Map<CoordinationGroup<TaskId, TaskId>, Set<TaskId>>
            mergeCoordinationProgress(Collection<PreparedTasks> preparedTaskGroups) {
        Map<TaskId, CoordinationGroup<TaskId, TaskId>> groupByLegacyTask = new HashMap<>();
        Map<CoordinationGroup<TaskId, TaskId>, Set<TaskId>> result = new HashMap<>();
        preparedTaskGroups.forEach(prepared -> prepared.migration.coordinationGroups.forEach(
                group -> {
                    CoordinationGroup<TaskId, TaskId> previous =
                            groupByLegacyTask.putIfAbsent(group.getKey(), group);
                    Preconditions.checkArgument(previous == null || previous.equals(group),
                            "Conflicting coordination manifests for legacy task %s",
                            group.getKey());
                    CoordinationGroup<TaskId, TaskId> canonical =
                            previous == null ? group : previous;
                    result.computeIfAbsent(canonical, ignored -> new HashSet<>());
                }));
        preparedTaskGroups.forEach(prepared ->
                prepared.migration.coordinationProgress.forEach((group, participants) -> {
                    CoordinationGroup<TaskId, TaskId> canonical =
                            Preconditions.checkNotNull(groupByLegacyTask.get(group.getKey()));
                    result.computeIfAbsent(canonical, ignored -> new HashSet<>())
                            .addAll(participants);
                }));
        return result;
    ...
    private void persistPreparedTasks(Collection<PreparedTasks> preparedTaskGroups) {
        // Merge and validate manifests before persisting anything. A malformed later group must
        // not leave an earlier group's tasks registered but not running.
        Map<CoordinationGroup<TaskId, TaskId>, Set<TaskId>> coordinationProgress =
                mergeCoordinationProgress(preparedTaskGroups);
        ...
        Set<TaskId> discoveredLegacyTasks = preparedTaskGroups.stream()
                .flatMap(prepared -> prepared.migration.legacyTasks.stream())
                .collect(Collectors.toSet());

        // Legacy absence proves its guarded cleanup is already complete (or was never needed).
        // Release any durable coordination record left by a crash or failure after source cleanup.
        coordinationProgress.keySet().stream()
                .filter(group -> !discoveredLegacyTasks.contains(group.getKey()))
                .forEach(group -> {
                    try {
                        workerConfiguration.transport.finishCoordination(group);
                    } catch (RuntimeException e) {
                        // The manifest remains in future assignments, so cleanup can be retried.
                        logger.atWarning().withCause(e).log(
                                "Could not release obsolete coordination state: %s", group);
                    }
                });

        if (!discoveredLegacyTasks.isEmpty()) {
            ...
            coordinationProgress.entrySet().stream()
                ...
                try {
                    if (workerConfiguration.transport.recordCompletion(
                        ...
                        coordinatedLegacyTasks.add(group.getKey());
                    }
                } catch (RuntimeException e) {
                    // Replacement states are already stored, so coordination can be retried on a
                    // later worker start without interrupting consumption.
                    logger.atWarning().withCause(e).log(
                            "Could not record coordination progress; keeping its source state: %s",
                            group);
                }
            });
            Set<TaskId> retiredLegacyTasks = new HashSet<>();
            coordinationProgress.keySet().stream()
                    .filter(group -> coordinatedLegacyTasks.contains(group.getKey()))
                    .forEach(group -> {
                        try {
                            workerConfiguration.transport.completeTaskStateMigration(
                                    Collections.singleton(group.getKey()));
                            retiredLegacyTasks.add(group.getKey());
                            try {
                                workerConfiguration.transport.finishCoordination(group);
                            } catch (RuntimeException e) {
                                logger.atWarning().withCause(e).log(
                                        "Could not release completed coordination state: %s",
                                        group);
                            }
                        } catch (RuntimeException e) {
                            // Completed coordination must remain retryable until source cleanup
                            // succeeds.
                            logger.atWarning().withCause(e).log(
                                    "Could not retire coordinated legacy tablet checkpoints; "
                                            + "keeping them for a later attempt: %s",
                                    group.getKey());
                        }
                    });

            Set<TaskId> pendingLegacyTasks = new HashSet<>(discoveredLegacyTasks);
            pendingLegacyTasks.removeAll(retiredLegacyTasks);
            if (!pendingLegacyTasks.isEmpty()) {
                logger.atFine().log(
                        "Legacy tablet checkpoints remain pending coordination: %s",
                        pendingLegacyTasks);
            }
            if (restoredTaskCount > 0) {
                logger.atInfo().log(
                        "Initialized %d replacement task states from legacy tablet checkpoints: %s",
                        restoredTaskCount, discoveredLegacyTasks);
            } else {
                logger.atFine().log(
                        "Legacy tablet checkpoints found, but all replacement states already "
                                + "existed: %s",
                        discoveredLegacyTasks);
            }
        }
    }
