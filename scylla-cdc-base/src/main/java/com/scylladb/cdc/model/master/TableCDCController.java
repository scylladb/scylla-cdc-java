...
public class TableCDCController {
    ...
    public boolean skipToNextGeneration() throws InterruptedException, ExecutionException {
        // Advance through done generations
        boolean anySkip = false;
        while (isGenerationDone()) {
            logger.atInfo().log("Current generation for table %s is done, advancing to next generation", table);
    ...
    public void runMasterStep() throws InterruptedException, ExecutionException {
        ...
        if (skipToNextGeneration()) {
            configureWorkers();
        }
    }

    /**
     * Returns the table name for this controller.
     */
    ...
    /**
     * Returns if the current generation is closed.
     */
    public boolean isGenerationClosed() {
        return currentGeneration.isClosed();
    }

    /**
     * Returns the current generation id.
     */
    public GenerationId getCurrentGenerationId() {
        return currentGeneration.getId();
    }
}
