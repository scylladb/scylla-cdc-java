...
public final class CatchUpConfiguration {
    ...
    private final AtomicReference<Optional<Date>> fixedCatchUpCutoff = new AtomicReference<>();
    ...
    /**
     * Computes the catch-up cutoff, freezing the value on first call.
     * Subsequent calls always return the same frozen value regardless of
     * the clock, ensuring Master and Worker use a consistent cutoff even
     * if they compute it at different times.
     */
    public Optional<Date> computeCatchUpCutoff(Clock clock) {
        Optional<Date> fixed = this.fixedCatchUpCutoff.get();
        if (fixed != null) {
            return fixed;
        }
        Optional<Date> computed = CatchUpUtils.computeCutoff(catchUpWindowSizeSeconds, clock);
        this.fixedCatchUpCutoff.compareAndSet(null, computed);
        return this.fixedCatchUpCutoff.get();
    }
    ...
    public static final class Builder {
        ...
        public void setCatchUpWindow(Duration catchUpWindow) {
            ...
            Preconditions.checkArgument(catchUpWindow.getNano() == 0,
                    "catchUpWindow must be an exact number of seconds, but got %s "
                    + "(nano component: %d); use Duration.ofSeconds(%d) or "
                    + "withCatchUpWindowSizeSeconds() instead",
                    catchUpWindow, catchUpWindow.getNano(), catchUpWindow.getSeconds());
