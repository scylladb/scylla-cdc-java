...
public final class Driver3WorkerCQL implements WorkerCQL {
    ...
    private final class Driver3Reader implements Reader {
        ...
        private void consume(ChangesConsumer consumer, CompletableFuture<Optional<ChangeId>> fut) {
            if (rs.getAvailableWithoutFetching() == 0) {
                ...
                Driver3Change change = new Driver3Change(rs.one());
                consumer.consume(change).thenRun(() -> fut.complete(Optional.of(change.getId())));
            }
        }

        @Override
        public CompletableFuture<Optional<ChangeId>> consume(ChangesConsumer consumer) {
            CompletableFuture<Optional<ChangeId>> result = new CompletableFuture<>();
            consume(consumer, result);
            return result;
        }
