 ...
 * only via the original {@link java.util.concurrent.ConcurrentHashMap}-based codepath. All progress
 * is lost on process restart in this mode. Provide a persistent implementation backed by Redis, a
 * SQL database, or any other durable store to allow the consumer to resume from the last checkpoint
 * after a restart. An {@link InMemoryStateStore} may also be passed explicitly for testing.
 *
