    ...
    try (Cluster cluster = Cluster.builder().addContactPoint(hostname).withPort(port).build()) {
      ...
      try (CDCConsumer consumer =
        ...
        String generationQuery = String.format(
            "SELECT COUNT(timestamp) FROM system.cdc_timestamps WHERE keyspace_name='%s' AND table_name='%s';",
            keyspace, table);
        ...
        long origGenerationCount = rs.one().getLong(0);
        Long generationCount = origGenerationCount;

        while (generationCount == origGenerationCount && (System.currentTimeMillis() - startTime) < timeoutMs) {
          rs = session.execute(generationQuery);
          generationCount = rs.one().getLong(0);
          Thread.sleep(1000); // Check every second
        }

        // Continue writing for a bit more after tablet alteration
        Thread.sleep(3000);
