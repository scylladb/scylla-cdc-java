...
/**
 * Unit tests for the state-delegation behaviour of {@link LocalTransport} when using
 * a {@link StoreBackedTaskStateBackend}.
 *
...
class LocalTransportWithStoreTest {
    ...
    void setUp() {
        ...
        transport = new LocalTransport(
                ...
                exec,
                new StoreBackedTaskStateBackend(store));
    ...
    void tabletMigration_persistsReplacementCheckpointsBeforeDeletingLegacyCheckpoint()
        ...
        try {
            worker.addTasks(coordinatedTasks(taskMap, generation, legacyTask));
    ...
    void tabletMigration_cleanupFailureDoesNotAbortAndIsRetried() throws Exception {
        ...
        try {
            firstWorker.addTasks(coordinatedTasks(taskMap, generation, legacyTask));
        ...
        try {
            restartedWorker.addTasks(coordinatedTasks(taskMap, generation, legacyTask));
    ...
    void constructor_loadsPersistedGenerationId_fromStore() {
        ...
        LocalTransport t = new LocalTransport(
                ...
                com.scylladb.cdc.model.worker.WorkerConfiguration.builder()
                        .withConsumer(change -> java.util.concurrent.CompletableFuture.completedFuture(null)),
                exec,
                new StoreBackedTaskStateBackend(seededStore));

        // getCurrentGenerationId() (vnode path) should return the persisted generation
    ...
    private static GroupedTasks coordinatedTasks(
        ...
        CoordinationGroup<TaskId, TaskId> group = new CoordinationGroup<>(
                ...
                legacyTask,
                tasks.keySet());
        return new GroupedTasks(tasks, generation, Collections.singleton(group));
    }
